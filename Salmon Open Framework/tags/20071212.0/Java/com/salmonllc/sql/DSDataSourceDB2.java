...
/**
 * Driver for DB2 Universal DB
 * @author   Mike Patnode (mike@mpsharp.com)
 */

class DSDataSourceDB2 extends DSDataSourceJDBC {

	/**
	 * DSDataSourceDB2 constructor comment.
	 */
	public DSDataSourceDB2() {
		super();
	}
	...
	/**Called by SOFIA to find an auto-generated Primary Key after it does an Insert.
	 * Note that you must call setAutoIncrement(pkey_col, true) and setUpdatable(pkey_col, false)
	 * for autogenerated pkeys to work.
	 */
	protected void populateAutoIncrementValue(DataStoreRow row, DBConnection conn, int colNo) {
		try {
			Statement st = conn.createStatement();

			// This is how you find the most-recently-added row's autonumber key in DB2.
			ResultSet r = st.executeQuery("values identity_val_local()");
			if (r.next()) {
				int colStat = row.getDSDataRow().getColumnStatus(colNo);
				int rowStat = row.getDSDataRow().getRowStatus();
				Object val = new Integer(r.getInt(1));
