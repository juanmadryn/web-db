...
public class AbmcSolicitudCompraController extends BaseEntityController {
	...
	public HtmlSubmitButton _copiarSolicitudCompraBUT1;
	...
	public void initialize() throws Exception {
		...
		_copiarSolicitudCompraBUT1 = new HtmlSubmitButton(
				"copiarSolicitudCompraBUT1", "Copiar solicitud", this);
		_copiarSolicitudCompraBUT1.setAccessKey("C");
		_detailformdisplaybox1.addButton(_copiarSolicitudCompraBUT1);
		...
		_generarCCBUT1 = new HtmlSubmitButton("generarCCBUT1",
				"Generar Cotización", this);
		...
		_copiarSolicitudCompraBUT1.addSubmitListener(this);
	...
	public boolean submitPerformed(SubmitEvent event) throws Exception {
		...
		if (component == _copiarSolicitudCompraBUT1) {
			conn = DBConnection.getConnection("inventario");
			conn.beginTransaction();
			try {
				// genero una nueva solicitud vacia.
				int solicitud_original_id = _dsSolicitudCompra.getRow();
				_dsSolicitudCompra.gotoRow(_dsSolicitudCompra.insertRow());
				_dsSolicitudCompra
				...
				_dsSolicitudCompra
						.setSolicitudesCompraUserIdSolicita(_dsSolicitudCompra
								.getSolicitudesCompraUserIdSolicita(solicitud_original_id));
				_dsSolicitudCompra.update(conn);
		...
		/**
		 * Agregado por Demian Barry el 21/05/2008, nueva funcionalidad de
		...
		if (component == _generarCCBUT1) {
			try {
				...
				_dsDetalleSC.filter(SELECCION_DETALLE_FLAG + " != null "
						+ "&& detalle_sc.cotizacion_compra_id == null");
				_dsDetalleSC.gotoFirst();
				...
				CotizacionesCompraModel dsCotizacionCompra = new CotizacionesCompraModel(
						"inventario");
				DetalleCotizacionModel dsDetalleCotizacion = new DetalleCotizacionModel(
						"inventario");
	...
	private void setDatosBasicosSolicitud() throws DataStoreException,
		...
		try {
			...
			// && (_dsSolicitudCompra.getRowStatus() !=
			// DataStore.STATUS_NEW_MODIFIED))
	...
	private void armaBotonera() throws DataStoreException, SQLException {
		...
		if (("0006.0003".equalsIgnoreCase(estado) || "0006.0008"
			...
			// asociado
			_dsDetalleSC
					.setFindExpression(DetalleSCModel.DETALLE_SC_ORDEN_COMPRA_ID
							+ " == null");
			if (_dsDetalleSC.findFirst())
				_generarOCBUT1.setVisible(true);

			// mostrar el boton de generar CC solo si hay articulos sin CC
			// asociada
			_dsDetalleSC
					.setFindExpression(DetalleSCModel.DETALLE_SC_COTIZACION_COMPRA_ID
							+ " == null");
			if (_dsDetalleSC.findFirst())
