 ...
 */
public class ConsultaRecepcionesComprasController extends BaseController {
	...
	public boolean submitPerformed(SubmitEvent e) throws Exception {
		...
		if (e.getComponent() == _searchformdisplaybox1.getSearchButton()) {
			...
			String where = _dsQBE.generateSQLFilter(_dsRecepciones);
			if (whereFecha != null) {
				if (where != null)
					where += " AND ";
				else
				...
				where += "";
			_dsRecepciones.retrieve(where);
			_dsRecepciones.gotoFirst();
		}

		if (e.getComponent() == _recuperaRecepcionesAnuladas) {
			_dsRecepciones.setRecepcionesComprasEstado("0009.0001");
