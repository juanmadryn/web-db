...
public class ConsultaArticulosController extends BaseController {
	...
	public inventario.models.ResumenSaldoArticulosModel dsResumen;
	...
	public void initialize() throws Exception {
		...
		// Obtiene operadores
		_operador.addOption(String.valueOf(BusquedaPorAtributo.OPERATOR_AND),
				"And");
		_operador.addOption(String.valueOf(BusquedaPorAtributo.OPERATOR_OR),
				"Or");

		// Test the import process
		_replicaFromTangoBUT = new HtmlSubmitButton("replicaFromTangoBUT",
				"Replicar desde Tango", this);
		_replicaFromTangoBUT.addSubmitListener(this);
		_searchformdisplaybox1.addButton(_replicaFromTangoBUT);

		_replicaProveedores = new HtmlSubmitButton("_replicaProveedores",
				"Importar proveedores", this);
		...
		_dsQBE.addCriteria("anulado", QBEBuilder.CRITERIA_TYPE_NOT_EQUALS,
				"articulos.anulado");
	...
	/**
	 * Genera el query SQL segùn los datos ingresados por el usuario en los
	 * campos asociados al QBE y los pares atributo/valor ingresados.
	 * 
	...
	private String armarCriterio() throws SQLException, DataStoreException {
		...
		String criterioAtributos = BusquedaPorAtributo
				.armarBusquedaPorAtributos(atributosValoresBusqueda(), Integer
						.valueOf(_operador.getValue()), "articulos",
						"articulo_id");
		if (criterioAtributos.length() > 0) {
			sb.append(" and articulos.articulo_id IN ( ").append(
	...
	/**
	 * Colecta en un hashtable los pares atributo/valor ingreados por el usuario
	 * para la bùsqueda
	 * 
	 * TODO: Al usar HashTable no se permiten claves duplicadas, por lo que no
	 * se puede buscar por multiples valores correspondientes a un solo
	 * atributo. Reemplazar por unas soluciòn "multimap" usando Map<key,List<value>>
	 * o un multimap de Apache Commons.
	 * 
	 * La cantidad de pares atributo/valor esta limitada al numero de
	 * componentes visuales agregados a tal efecto. Generar estos componentes
	 * dinamicamente.
	 * 
	 * @return hashtable
	 */
