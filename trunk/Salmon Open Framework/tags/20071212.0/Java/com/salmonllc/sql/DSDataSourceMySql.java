 ...
 */
class DSDataSourceMySql extends DSDataSourceJDBC {

	public DSDataSourceMySql() {
		super();
		_fixDashes=true;
	}
	...
	public String generateSelect(DataStore ds, String criteria, boolean countOnly) throws DataStoreException {
		...
		if (countOnly)
			...
			for (int i = 0; i < ds.getColumnCount(); i++) {
				...
				if (databaseName != null) {
					colList.append(fixDashes(databaseName));
					colList.append(",");
				}
			}
		...
		for (int i = 0; i < ds.getJoinCount(); i++) {
			...
			if (index > -1) {
				if (aliasUsed[index])
					...
					if (ds.getAlias(index)	!= null) 
						leftTable = ds.getTable(index) + " " + leftTable;
			...
			if (index > -1) {
                if (aliasUsed[index] && ! ds.getJoinOuter(i)) {
				    ...
				    if (ds.getAlias(index)	!= null)
					    rightTable = ds.getTable(index) + " " + rightTable;
                }
                aliasUsed[index] = true;
			}
			joinClause.append(" " + fixDashes(leftTable) + join + fixDashes(rightTable) + " ON ");
			for (int j = 0; j < ds.getJoinColumnCount(i); j++) {
				if (j > 0)
					joinClause.append(" AND ");
				joinClause.append(fixDashes(ds.getJoinLeftColumn(i, j)) + " = " + fixDashes(ds.getJoinRightColumn(i, j)));
			}
		}
		StringBuffer fromClause = new StringBuffer();
		fromClause.append(" FROM ");
		if ((ds.getDefaultTable() != null) && (!aliasUsed[ds.getAliasCount()]))
			fromClause.append(fixDashes(ds.getDefaultTable()) + ",");
		for (int i = 0; i < ds.getAliasCount(); i++) {
			if (!aliasUsed[i]) {
				fromClause.append(fixDashes(ds.getTable(i)));
				if (ds.getAlias(i) != null)
					fromClause.append(" " + fixDashes(ds.getAlias(i)));
				fromClause.append(",");
			}
		}
		...
		if (ds.getGroupBy() != null)
			retVal += " GROUP BY " + fixDashes(ds.getGroupBy());
		if (ds.getHaving() != null)
			retVal += " HAVING " + fixDashes(ds.getHaving());
		if (ds.getOrderBy() != null && !countOnly)
			retVal += " ORDER BY " + fixDashes(ds.getOrderBy());
		return retVal;
