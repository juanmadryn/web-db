 ...
 */
public class ProyectoModel extends DataStore {
	...
	public ProyectoModel(String appName, String profile) {
		...
		try {
			...
			addColumn(computeTableName("infraestructura.entidad_externa"),
					"codigo", DataStore.DATATYPE_STRING, false, false,
			...
			addLookupRule(
					...
					"nombre", "entidad_externa.nombre", "Cliente inexistente");
	...
	public void update(DBConnection conn, boolean handleTrans)
		...
		try {
			...
			_pagina
					.displayErrorMessage("Error determinando circuito y estado inicial para los proyectos");
		} finally {
			if (r != null) {
				try {
					r.close();
