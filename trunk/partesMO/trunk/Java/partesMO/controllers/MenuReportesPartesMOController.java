//package statement
package partesMO.controllers;

//Salmon import statements
import com.salmonllc.html.events.PageEvent;

import infraestructura.controllers.BaseController;

/**
 * MenuReportesPartesEqController: a SOFIA generated controller
 */
public class MenuReportesPartesMOController extends BaseController {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;

	// Visual Components
	public com.salmonllc.html.HtmlImage _reporte1IMG1;
	public com.salmonllc.html.HtmlImage _reporte2IMG1;
	public com.salmonllc.html.HtmlText _reporte1TXT1;
	public com.salmonllc.html.HtmlText _reporte2TXT1;
	public com.salmonllc.html.HtmlImage _reporte1IMG2;
	public com.salmonllc.html.HtmlImage _reporte2IMG2;
	public com.salmonllc.html.HtmlText _reporte1TXT2;
	public com.salmonllc.html.HtmlText _reporte2TXT2;
	public com.salmonllc.html.HtmlText _text1Footer;
	public com.salmonllc.html.HtmlText _text2;
	public com.salmonllc.html.HtmlText _text2Footer;
	public com.salmonllc.html.HtmlText _text3Footer;
	public com.salmonllc.jsp.JspLink _reporte1BUT2;
	public com.salmonllc.jsp.JspLink _reporte2BUT2;
	public com.salmonllc.jsp.JspLink _reporte1BUT1;
	public com.salmonllc.jsp.JspLink _reporte2BUT1;
	public com.salmonllc.html.HtmlTextEdit _fechadesde2;
	public com.salmonllc.html.HtmlTextEdit _fechahasta2;

	/**
	 * Initialize the page. Set up listeners and perform other initialization
	 * activities.
	 * 
	 * @throws Exception
	 */
	public void initialize() throws Exception {
		super.initialize();		
	}

	@Override
	public void pageRequested(PageEvent p) throws Exception {
		// TODO Auto-generated method stub
		String URL = armarUrlReporte("PDF", "partesmo_por_fecha",  "&param_fecha_desde="+_fechadesde2.getValue()+"&param_fecha_hasta="+_fechahasta2.getValue());
		_reporte1BUT2.setHref(URL);
		
		URL = armarUrlReporte("XLS", "partesmo_por_fecha", "&param_fecha_desde="+_fechadesde2.getValue()+"&param_fecha_hasta="+_fechahasta2.getValue());
		_reporte2BUT2.setHref(URL);		
		super.pageRequested(p);
	}
	
	
}
