...
public class AccesoMenuModel extends DataStore {
	...
	private static UsuarioRolesModel roles;
	...
	public AccesoMenuModel(String appName, String profile) {
		...
		try {
			...
			// add columns
			addColumn(computeTableName("acceso_menu"), "menu_id",
					DataStore.DATATYPE_INT, false, true, ACCESO_MENU_MENU_ID);
			addColumn(computeTableName("acceso_menu"), "rol_id",
					DataStore.DATATYPE_INT, false, true, ACCESO_MENU_ROL_ID);
			addColumn(computeTableName("acceso_menu"), "user_id",
					DataStore.DATATYPE_INT, false, true, ACCESO_MENU_USER_ID);
			addColumn(computeTableName("website_user"), "login_name",
					DataStore.DATATYPE_STRING, false, false, WEBSITE_USER_LOGIN_NAME);
			addColumn(computeTableName("website_user"), "nombre_completo",
					DataStore.DATATYPE_STRING, false, false, WEBSITE_USER_APEYNOM);
			addColumn(computeTableName("roles"), "nombre",
					DataStore.DATATYPE_STRING, false, false, ROLES_NOMBRE);
			addColumn(computeTableName("menu"), "nombre",
					DataStore.DATATYPE_STRING, false, false, MENU_NOMBRE);
			addColumn(computeTableName("menu"), "descripcion",
					DataStore.DATATYPE_STRING, false, false, MENU_DESCRIPCION);
			addColumn(computeTableName("menu"), "grupo",
					DataStore.DATATYPE_STRING, false, false, MENU_GRUPO);
			addColumn(computeTableName("acceso_menu"), "acceso_menu_id",
					DataStore.DATATYPE_INT, true, true, ACCESO_MENU_ACCESO_MENU_ID);

			// add joins
			addJoin(computeTableAndFieldName("acceso_menu.user_id"),
					computeTableAndFieldName("website_user.user_id"), true);
			addJoin(computeTableAndFieldName("acceso_menu.rol_id"),
					computeTableAndFieldName("roles.rol_id"), true);
			addJoin(computeTableAndFieldName("acceso_menu.menu_id"),
					computeTableAndFieldName("menu.menu_id"), false);

			// add validations
			addLookupRule(WEBSITE_USER_LOGIN_NAME, "website_user",
					"'user_id = ' + acceso_menu.user_id", "user_id",
					"website_user.login_name", "Usuario inexistente");
			addLookupRule(ROLES_NOMBRE, "roles",
					"'rol_id = ' + acceso_menu.rol_id", "rol_id", "roles.nombre",
					"Rol inexistente");
			addLookupRule(MENU_NOMBRE, "menu",
					"'menu_id = ' + acceso_menu.menu_id", "menu_id", "menu.nombre",
					"Menú inexistente");
	...
	/**
	 * Retrieve the value of the acceso_menu.menu_id column for the specified
	 * row.
	 * 
	 * @param row
	 ...
	 * @throws DataStoreException
	 */
	public void setAccesoMenuMenuId(int row, int newValue)
			throws DataStoreException {
	 ...
	 * @throws DataStoreException
	 */
	public void setAccesoMenuRolId(int row, int newValue)
			throws DataStoreException {
	...
	/**
	 * Retrieve the value of the acceso_menu.user_id column for the specified
	 * row.
	 * 
	 * @param row
	 *           which row in the table
	 ...
	 * @throws DataStoreException
	 */
	public void setAccesoMenuUserId(int row, int newValue)
			throws DataStoreException {
	...
	/**
	 * Retrieve the value of the website_user.login_name column for the specified
	 * row.
	 * 
	 * @param row
	 *           which row in the table
	 ...
	 * @throws DataStoreException
	 */
	public void setRolesNombre(int row, String newValue)
			throws DataStoreException {
	 ...
	 * @throws DataStoreException
	 */
	public void setMenuNombre(int row, String newValue)
			throws DataStoreException {
	 ...
	 * @throws DataStoreException
	 */
	public void setMenuDescripcion(int row, String newValue)
			throws DataStoreException {
	...
	/**
	 * Retrieve the value of the acceso_menu.acceso_menu_id column for the
	 * current row.
	 * 
	 * @return int
	 * @throws DataStoreException
	...
	/**
	 * Set the value of the acceso_menu.acceso_menu_id column for the current
	 * row.
	 * 
	 * @param newValue
	 *           the new item value
	 * @throws DataStoreException
	 */
	public void setAccesoMenuAccesoMenuId(int newValue)
			throws DataStoreException {
		setInt(ACCESO_MENU_ACCESO_MENU_ID, newValue);
	}

	/**
	 * Set the value of the acceso_menu.acceso_menu_id column for the specified
	 * row.
	 * 
	 * @param row
	 *           which row in the table
	 ...
	 * @throws DataStoreException
	 */
	public void setAccesoMenuAccesoMenuId(int row, int newValue)
			throws DataStoreException {
	...
	public boolean allowedMenuByUser(int user_id, int menu_id)
			throws DataStoreException, SQLException {
		filter(null);

		if (roles == null) {
			roles = new UsuarioRolesModel("infraestructura", "infraestructura");
		}
		
		roles.filter("usuario_roles.user_id ==" + user_id);
		
		for (int row = 0; row < roles.getRowCount(); row++) {
			filter("acceso_menu.rol_id ==" + roles.getUsuarioRolesRolId(row));			
			for (int row2 = 0; row2 < getRowCount(); row2++) {				
				if (getAccesoMenuMenuId(row2) == menu_id) {
					return true;
				}
			}
		}

		filter("acceso_menu.user_id ==" + user_id);

		for (int row = 0; row < getRowCount(); row++) {
			if (getAccesoMenuAccesoMenuId(row) == menu_id) {
				return true;
			}
		}
		return false;
	...
	public void reloadRoles() throws DataStoreException, SQLException {		
		if (roles == null) {
			roles = new UsuarioRolesModel("infraestructura", "infraestructura");
		}
		roles.retrieve();
