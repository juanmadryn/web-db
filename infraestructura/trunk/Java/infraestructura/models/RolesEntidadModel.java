...
public class RolesEntidadModel extends DataStore {
	...
	//constants for columns
      ...
      * SÛlo setea el default si no est√° definido a√∫n 
      * @author demian
      * @param row indica sobre que fila del datastor se realiza la operaciÛn
      */
     public void setColumnasDefault(int row, int entidad_id) throws DataStoreException {
    	 // le asigno la entidad (maestro), sÛlo si no est√° seteada a√∫n
     ...
     /**
      * Realiza el proceso de activaciÛn de una entidad
     ...
     public void activarEntidad(int row) throws DataStoreException, SQLException {
		...
		if (estado == "V") {
			throw new DataStoreException("El rol para la entidad YA est√° activa");
		}
