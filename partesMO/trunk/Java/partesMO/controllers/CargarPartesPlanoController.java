 ...
 */
public class CargarPartesPlanoController extends BaseController implements
	...
	public void initialize() throws Exception {
		...
		_nuevoParteCopiarBUT1 = new HtmlSubmitButton("nuevoParteCopiarBUT1",
				"Copiar Parte", this);
		...
		_nuevoParteNuevoBUT2 = new HtmlSubmitButton("nuevoParteNuevoBUT2",
				"Nuevo Parte", this);
		...
		if ("false".equalsIgnoreCase(Props.getProps("partesmo", null)
	...
	public void refrescaPartes() throws SQLException, DataStoreException {
		...
		// si no recuperó ningún parte en estado generado, inserta al menos un
		// registro
	...
	public boolean submitPerformed(SubmitEvent e) throws Exception {
		...
		if (e.getComponent() == _nuevoParteCopiarBUT1
				|| e.getComponent() == _nuevoBUT91) {
		...
		if (e.getComponent() == _nuevoParteNuevoBUT2
				|| e.getComponent() == _nuevoBUT92) {
			...
			if (row > 1) {
				...
				_dsPartes.setPartesMoFecha(row, _dsPartes
						.getPartesMoFecha(rowActual + 1));
				_dsPartes.setPartesMoProyectoId(row, _dsPartes
						.getPartesMoProyectoId(rowActual + 1));
			}
			...
			if (_dsPartes != null) {
				if (!_dsPartes.isFormattedStringValid(e.getColumn(), e
					...
					if (_dsPartes.getMensajeError(_dsPartes.getRow()) == null)
						_dsPartes.setMensajeError(_dsPartes.getRow(),
								"Formato de fecha incorrecto.");
					else
						_dsPartes.setMensajeError(_dsPartes.getRow(), _dsPartes
								.getMensajeError(_dsPartes.getRow())
								+ " - Formato de fecha incorrecto.");
	...
	public void completaCategorias() throws DataStoreException {
		...
		try {
			...
			throw new DataStoreException(
					"Error determinando categorías en tango: " + e.getMessage());
	...
	public void pageRequested(PageEvent p) throws Exception {
		...
		if (!isReferredByCurrentPage()) {
			...
			if (v_parte_id > 0) {
				...
				_dsPartes.retrieve("partes_mo.parte_id = "
						+ Integer.toString(v_parte_id));
				_dsPartes.gotoFirst();
